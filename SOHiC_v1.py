...
# Sidebar - Specify parameter settings for the FP
st.sidebar.header('Morgan FP Radio')
fingerprint_radio = st.sidebar.slider('Set the Morgan fingerprint Radio', 2, 4, 2, 1)

st.sidebar.subheader('Morgan FP Lengh')
fingerprint_lenght = st.sidebar.slider('Set the Morgan fingerprint lenght', 512, 2048, 1024, 512)

st.sidebar.subheader('Morgan FP with Features')
features = st.sidebar.checkbox('Check if you want Morgan fingerprint to be generated with Features', True, False)

# Method by which the distance matrix is recalculated once each cluster is formed. More info: https://docs.scipy.org/doc/scipy/reference/generated/scipy.cluster.hierarchy.linkage.html
st.sidebar.subheader('Clustering method')
metodo = st.sidebar.selectbox("Choose the clustering method", ("single", "complete", "average", "weighted"),0)
...
# If "ready = True", set your distance selected cutoff
if ready == True:
...
def silhouette_plot(df, maximo, limit_dense):
    ...
    fig.update_layout(plot_bgcolor = 'rgb(256,256,256)', hovermode='x', legend_x = 1.1,
                      legend = dict( font = dict(size=18, family='Calibri', color='black')))
...
def dendogram_plot(ddgm, distanceMatrix):
    ...
    fig2.update_layout(plot_bgcolor = 'rgb(256,256,256)')
...
def filedownload(df_FP, ddgm, list_smiles):
    ...
    bar_plot_counts(clusters_final)
